...
In this non-student dataset, at F(1,16)=5.55, p=0.032, we can conclude that the number of correctly recalled words is significantly associated with valence but at F(1,16)=0.82, p=0.38, it is not significantly associated with the level of arousal. Additionally, at F(1,15)=0.12, p=0.74, there were no interactions between valence and arousal.

Since there was a significant association between non-students' recollected words and valence, I wanted to be certain that student status was, indeed, the issue at hand. An additional column was added to affect.csv and each entry was coded with 'y' for student and 'n' for nonstudent.  Another two-way between-subjects ANOVA will be run...

```{r}
summary(aov(correct~student*valence*arousal,data=affect))
```

This analysis clearly shows, at F(1,35)=5.172, p=0.03, that the number of correctly recalled words is significantly associated with student status and that no other significant interactions were observed.
